webpackHotUpdate("static/development/pages/_app.js",{

/***/ "./components/Footer.js":
/*!******************************!*\
  !*** ./components/Footer.js ***!
  \******************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! react */ "./node_modules/react/index.js");
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(react__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var styled_components__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! styled-components */ "./node_modules/styled-components/dist/styled-components.browser.esm.js");
/* harmony import */ var _GoogleMaps__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./GoogleMaps */ "./components/GoogleMaps.js");
var _jsxFileName = "/mnt/c/Users/D.Hardiman/desktop/eurobrake/components/Footer.js";
var __jsx = react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement;



var FooterBar = styled_components__WEBPACK_IMPORTED_MODULE_1__["default"].div.withConfig({
  displayName: "Footer__FooterBar",
  componentId: "sc-1wn18ws-0"
})(["color:", ";background:", ";min-width:100%;min-height:10rem;display:flex;justify-content:space-between;align-items:flex-end;position:absolute;bottom:0;padding:2rem;.ftr_container-1{padding:.2rem;min-width:30rem;display:flex;flex-flow:column;}.ftr_container-2{padding:.2rem;min-width:20rem;min-height:10rem;display:flex;flex-flow:column;justify-content:space-between;align-items:center;}.ftr_container-3{display:flex;flex-flow:column;align-items:center;}.ftr_newsletter-box{padding:1rem;min-width:40rem;display:flex;flex:1;}.ftr_container-5{padding:1rem;min-width:40rem;border:1px solid white;min-height:20rem;display:flex;flex-flow:column;}.ftr_sponsor-box{padding:1rem;min-width:40rem;border:1px solid white;min-height:25rem;display:flex;flex-flow:column wrap;}.ftr_container-7{display:flex;position:relative;}.social{color:", ";;margin:1rem;font-size:2rem;}a{color:", ";text-transform:uppercase;font-family:", ";font-size:2rem;}h1{padding:1rem;text-align:center;}input{margin-left:auto;}label{font-family:", ";font-size:1.6rem;text-transform:uppercase;}"], function (props) {
  return props.theme.white;
}, function (props) {
  return props.theme.primary;
}, function (props) {
  return props.theme.white;
}, function (props) {
  return props.theme.white;
}, function (props) {
  return props.theme.MPBold;
}, function (props) {
  return props.theme.MPBold;
});

function Footer() {
  return __jsx(FooterBar, {
    __source: {
      fileName: _jsxFileName,
      lineNumber: 129
    },
    __self: this
  }, __jsx("div", {
    className: "ftr_container-1",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 130
    },
    __self: this
  }, __jsx("h1", {
    __source: {
      fileName: _jsxFileName,
      lineNumber: 131
    },
    __self: this
  }, "CONFIRMED EUROBRAKE 2020 SPONSORS"), __jsx("div", {
    className: "ftr_sponsor-box",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 132
    },
    __self: this
  }, "SPONSOR BOX")), __jsx("div", {
    className: "ftr_container-2",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 134
    },
    __self: this
  }, __jsx("a", {
    href: "https://www.eurobrake.net/privacy",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 135
    },
    __self: this
  }, "Privacy Policy"), __jsx("a", {
    href: "https://www.eurobrake.net/disclaimer",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 136
    },
    __self: this
  }, "Disclaimer"), __jsx("a", {
    href: "",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 137
    },
    __self: this
  }, "Contact Us"), __jsx("div", {
    className: "ftr_container-7",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 140
    },
    __self: this
  }, __jsx("a", {
    className: "social",
    href: "http://www.linkedin.com/groups/EuroBrake-4282205?trk=myg_ugrp_ovr",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 141
    },
    __self: this
  }, __jsx("img", {
    src: "/images/Linkedin@2000x.png",
    alt: "Linkedin",
    style: {
      width: '5rem'
    },
    __source: {
      fileName: _jsxFileName,
      lineNumber: 142
    },
    __self: this
  })), __jsx("a", {
    className: "social",
    href: "http://www.facebook.com/pages/FISITA-International-Federation-of-Automotive-Engineering-Societies/123022704532",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 144
    },
    __self: this
  }, __jsx("img", {
    src: "/images/Facebook@2000x.png",
    alt: "Facebook",
    style: {
      width: '5rem'
    },
    __source: {
      fileName: _jsxFileName,
      lineNumber: 145
    },
    __self: this
  })), __jsx("a", {
    className: "social",
    href: "http://twitter.com/FISITAhq",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 147
    },
    __self: this
  }, __jsx("img", {
    src: "/images/Twitter@2000x.png",
    alt: "Twitter",
    style: {
      width: '5rem'
    },
    __source: {
      fileName: _jsxFileName,
      lineNumber: 148
    },
    __self: this
  })), __jsx("a", {
    className: "social",
    href: "https://www.flickr.com/photos/eurobrake/albums/72157681368290774",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 150
    },
    __self: this
  }, __jsx("img", {
    src: "/images/Facebook@2000x.png",
    alt: "Flickr",
    style: {
      width: '5rem'
    },
    __source: {
      fileName: _jsxFileName,
      lineNumber: 151
    },
    __self: this
  })))), __jsx("div", {
    className: "ftr_container-3",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 155
    },
    __self: this
  }, __jsx("div", {
    className: "ftr_newsletter-box",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 157
    },
    __self: this
  }, __jsx("label", {
    htmlFor: "Newsletter",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 158
    },
    __self: this
  }, "Newsletter"), __jsx("input", {
    type: "email",
    name: "Newsletter",
    id: "",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 159
    },
    __self: this
  })), __jsx("div", {
    className: "ftr_container-7",
    __source: {
      fileName: _jsxFileName,
      lineNumber: 161
    },
    __self: this
  }, __jsx(_GoogleMaps__WEBPACK_IMPORTED_MODULE_2__["default"], {
    __source: {
      fileName: _jsxFileName,
      lineNumber: 162
    },
    __self: this
  }))));
}

/* harmony default export */ __webpack_exports__["default"] = (Footer);
{
  /* <div className="ftr_container-5">Map Box</div> */
}

/***/ })

})
//# sourceMappingURL=_app.js.61fa70018ab98272d10c.hot-update.js.map